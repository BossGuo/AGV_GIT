  ...
  <div class="midiv" v-show="state.ishow">
    <div class="top">
      ...
      <div style="float:left;">
        ...
        <el-select v-model="state.countortime_ssyx" placeholder="请选择" size="mini" style="width:160px;float:left;" @change="countortime_ssyxChange">
          <el-option v-for="item in state.countortime_ssyx_data" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
        <div style="float:left;color:white;line-height:2.6vh;">
          =>
          <el-input-number v-model.lazy="countortime_ssyx_val" :min="0" :max='9999' @change="countortime_ssyx_value_Change" size='mini' />
        </div>
        <div style="float:left;color:white;margin-left:1vw;line-height:2.6vh;">
          日志级别:&nbsp;
        </div>
        <el-select :clearable='true' multiple v-model="state.Lvl" placeholder="请选择" size="mini" style="width:160px;float:left;">
          <el-option v-for="item in state.Lvl_opts" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
        <div style="float:left;color:white;margin-left:1vw;line-height:2.6vh;">
          日志来源:&nbsp;
    ...
    <div class="eltable">
      <el-table
        :data="state.tableData.filter(x=>x.Type.indexOf(state.Type)>=0&&x.Lvl.indexOf(state.Lvl)>=0&&x.Sender.indexOf(state.Sender)>=0).slice((state.PageNum - 1) * state.PageSize, (state.PageNum * state.PageSize) - 1)"
...
const formatter_logtype = (val) => {
  return val == 'AGVCmd' ? 'AGV交互指令' : (val == 'UserOperation' ? '用户操作' : '系统运行日志')
}
const countortime_ssyx_val = ref(100);
const state = reactive({
  tableData: computed(() => {
    return store.state.runtime_log_ssyxzdata;
  }),
  Sender: '',
  Type: "",
  Type_opts: [
  ...
  ishow: true,
  countortime_ssyx: 'count',
  countortime_ssyx_val: 10,
  countortime_ssyx_data: [
    {
      label: '最大记录时长(分钟)',
      value: 'time'
    },
  ...
  totalCount: computed(() => {
    return store.state.runtime_log_ssyxzdata.length;
  }),
  PageNum: 1,
  PageSize: 10,
});
const countortime_ssyxChange = (val) => {
  setCookie('countortime_ssyx', val);
  setCookie('countortime_ssyx_val', 0);
  countortime_ssyx_val.value = 0;
}
const handleCurrentChange = (val) => {
  state.PageNum = val;
}
const countortime_ssyx_value_Change = (val) => {
  setCookie('countortime_ssyx_val', val);
}
...
onBeforeMount(() => {
  ...
  state.countortime_ssyx = getCookie('countortime_ssyx');
  countortime_ssyx_val.value = parseInt(getCookie('countortime_ssyx_val'));
})
</script>
