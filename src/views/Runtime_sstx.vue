  ...
  <div class="midiv" v-show="state.ishow">
    <div class="top">
      ...
      <div style="float:left;">
        ...
        <el-select v-model="state.countortime_sstx" placeholder="请选择" size="mini" style="width:160px;float:left;" @change="countortime_sstxChange">
          <el-option v-for="item in state.countortime_sstx_data" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
        <div style="float:left;color:white;line-height:2.6vh;">
          =>
          <el-input-number v-model.lazy="countortime_sstx_val" :min="0" :max='9999' @change="countortime_sstx_value_Change" size='mini' />
        </div>
        <div style="float:left;color:white;margin-left:1vw;line-height:2.6vh;">
          日志级别:&nbsp;
        </div>
...
const countortime_sstx_val = ref(100);
const state = reactive({
  tableData: computed(() => {
    return store.state.runtime_log_sstxdata;
  }),
  Sender: '',
  Type: "",
  Type_opts: [
  ...
  ishow: true,
  countortime_sstx: 'count',
  countortime_sstx_val: 10,
  countortime_sstx_data: [
    {
      label: '最大记录时长(分钟)',
      value: 'time'
    },
  ...
  totalCount: computed(() => {
    return store.state.runtime_log_sstxdata.length;
  }),
  PageNum: 1,
  PageSize: 10,
});
const countortime_sstxChange = (val) => {
  setCookie('countortime_sstx', val);
  setCookie('countortime_sstx_val', 0);
  countortime_sstx_val.value = 0;
}
const handleCurrentChange = (val) => {
  state.PageNum = val;
}
const countortime_sstx_value_Change = (val) => {
  setCookie('countortime_sstx_val', val);
}
...
onBeforeMount(() => {
  ...
  state.countortime_sstx = getCookie('countortime_sstx');
  countortime_sstx_val.value = parseInt(getCookie('countortime_sstx_val'));
})
</script>
